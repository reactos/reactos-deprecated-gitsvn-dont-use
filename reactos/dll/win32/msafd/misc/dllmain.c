          ...
          LPINT lpErrno)
{
    ...
    HANDLE                      Sock;
    PSOCKET_INFORMATION         Socket = NULL, DuplicatedSocket = NULL;
    ...
    {
        DuplicatedSocket = GetSocketStructure(lpProtocolInfo->dwServiceFlags4);
        if (!DuplicatedSocket)
        {
            if (lpErrno)
                *lpErrno = WSAENOTSOCK;
        ...
        RtlCopyMemory(Socket, DuplicatedSocket, sizeof(*Socket));
        RtlCopyMemory(&Socket->ProtocolInfo, &DuplicatedSocket->ProtocolInfo, sizeof(Socket->ProtocolInfo));
        Sock = (HANDLE)lpProtocolInfo->dwServiceFlags3;
        Socket->Handle = lpProtocolInfo->dwServiceFlags3;
        Socket->LocalAddress = &Socket->SharedData->WSLocalAddress;
        Socket->RemoteAddress = &Socket->SharedData->WSRemoteAddress;
        Socket->SanData = NULL;
        InterlockedIncrement(&Socket->SharedData->RefCount);
        goto ok;
    }
    ...
    OUT LPINT lpErrno)
{
    ...
    lpProtocolInfo->dwServiceFlags3 = (DWORD)NewSocket;
    lpProtocolInfo->dwServiceFlags4 = Socket->Handle;
...
ok:
    ...
    if (!References) HeapFree(GlobalHeap, 0, Socket->SharedData);
    HeapFree(GlobalHeap, 0, Socket);
    return MsafdReturnWithErrno(Status, lpErrno, 0, NULL);
}
          ...
          LPINT lpErrno)
{
    ...
    AcceptSocketInfo->State = SocketConnected;
